function Res = dfmFULLVAR1(X,Spec,threshold)
...
% end
nf=sum(r)*(ppC-1);
EZZ = Zsmooth(1:ppC:nf,2:end)*Zsmooth(1:ppC:nf,2:end)'+sum(Vsmooth(1:ppC:nf,1:ppC:nf,2:end),3);                        %E(Z'Z)
EZZ_BB = Zsmooth(1:ppC:nf,1:end-1)*Zsmooth(1:ppC:nf,1:end-1)'+sum(Vsmooth(1:ppC:nf,1:ppC:nf,1:end-1),3); %E(Z(-1)'Z_(-1))
EZZ_FB = Zsmooth(1:ppC:nf,2:end)*Zsmooth(1:ppC:nf,1:end-1)'+sum(VVsmooth(1:ppC:nf,1:ppC:nf,:),3);%E(Z'Z_(-1)) 

% VAR for the common factors
A_new(1:ppC:nf,1:ppC:nf) = EZZ_FB * inv(EZZ_BB);
Q_new(1:ppC:nf,1:ppC:nf) = (EZZ - A_new(1:ppC:nf,1:ppC:nf)*EZZ_FB') / T;
temp=Q_new(1:sum(r)*ppC,1:sum(r)*ppC);
initV_i = reshape(inv(eye((sum(r)*ppC)^2)-kron(A_new(1:sum(r)*ppC,1:sum(r)*ppC),A_new(1:sum(r)*ppC,1:sum(r)*ppC)))*temp(:),sum(r)*ppC,sum(r)*ppC);
V_0_new(1:sum(r)*ppC,1:sum(r)*ppC) = initV_i;
...
A = zeros(sum(r)*ppC,sum(r)*ppC)';  % Initialize transition matrix
Eye=ones(sum(r)*ppC-1,1);
Eye(ppC:ppC:end,:)=0;
A(2:end,1:end-1)=diag(Eye);
A_temp = (inv(Z'*Z)*Z'*z)';  % OLS: gives coefficient value AR(p) process
A(1:ppC:end,1:ppC:end)=A_temp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Q = zeros(ppC*sum(r),ppC*sum(r));
e = z  - Z*A_temp';         % VAR residuals
Q_temp = cov(e); % VAR covariance matrix
Q(1:ppC:end,1:ppC:end)=Q_temp;

initV_i = reshape(inv(eye((sum(r)*ppC)^2)-kron(A,A))*Q(:),sum(r)*ppC,sum(r)*ppC);
