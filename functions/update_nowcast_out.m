function[y_old,y_new,news_table,revision,impact_revisions] = update_nowcast_out(X_old,X_new,Time,Spec,Res,series,period,vintage_old,vintage_new)
...
% [y_old,~,~,actual_old,forecast_old,weight_old,t_miss2,v_miss2,innov_old] = News_DFM2(X_old,X_rev,Res,t_nowcast,i_series);
[y_old] = News_DFM(X_old,X_rev,Res,t_nowcast,i_series);

revision = X_rev - X_old;

% Compute impact from data releases
% [y_rev,y_new,~,actual,forecast,weight,~,~,~,forecast_base,weight_base] = News_DFM(X_rev,X_new,Res,t_nowcast,i_series,t_miss2,v_miss2,innov_old);
...
%     revisions = forecast - forecast_old;              % News from revisions
%     revisions = forecast_base - forecast_old;              % News from revisions
%     revisions = actual_old - forecast_old;              % News from revisions
    
    impact_releases = weight .* (news);    % Impact of releases
%     impact_revisions_ind = weight_old .* (nansum(revisions)');    % Impact of revisions
%     impact_revisions_ind = weight .* (revisions);    % Impact of revisions
...
%     rev_table = array2table([forecast_old, forecast_base, weight_base, impact_revisions_ind],...
%                             'VariableNames', {'Previous', 'Actual', 'Weight', 'Impact'},...
%                             'RowNames', Spec.SeriesID);
...
%     rev_index = find(nansum(revision));
    % Display the table output
    fprintf('\n  Nowcast Detail Table \n\n')
    disp(news_table(data_released, :))
%     disp(rev_table(rev_index, :))

% end

% Output results to structure

% table_release = news_table(data_released, :);
% table_revision = revision_table;
...
%     Wx_old = Res_old.Wx;
%     C_old = Res_old.C;   % Observation matrix
    
%     actual=X_new(t_fcst,v_news);forecast=X_old(t_fcst,v_news);
        actual     = NaN(N,1);  % Actual forecasted values
        forecast   = NaN(N,1);  % Forecasted values
        weight     = NaN(N,1);  % Forecasted values
%     actual=[];forecast=[];weight=[];
    ...
    % Time/variable indices where case (ii) is true
...
function [y_old,y_new,singlenews,actual,forecast,weight,t_miss2,v_miss2,innov] = News_DFM2(X_old,X_new,Res,t_fcst,v_news)
%News_DFM()    Calculates changes in news
%             
%  Syntax:
%  [y_old, y_new, singlenews, actual, fore, weight ,t_miss, v_miss, innov] = ...
...
%     actual=X_new(t_fcst,v_news);forecast=X_old(t_fcst,v_news);
    actual=[];forecast=[];weight=[];t_miss=[];v_miss=[];innov=[];t_miss2=[];v_miss2=[];
    ...
    miss_old=isnan(X_old);
    miss_new=isnan(X_new);
%     data_release = X_new - X_old;
    
    % Indicator for missing--combine above information to single matrix where:
    % (i) -1: Value is in the old data, but missing in new data
    % (ii) 1: Value is in the new data, but missing in old data 
    % (iii) 0: Values are missing from/available in both datasets
    % (iv) : value i different in new
    i_miss = miss_old - miss_new;
%     i_miss_temp1 = data_release>0; i_miss_temp2 = data_release<0;
%     i_miss = i_miss_temp1 + i_miss_temp2;
...
%     actual=[];
    data_release = X_new - X_old;
    i_miss_temp1 = data_release>0; i_miss_temp2 = data_release<0;
    i_miss2 = i_miss_temp1 + i_miss_temp2;
    [t_miss2,v_miss2]=find(i_miss2==1);
    for i=1:size(t_miss2,1)
        % Standardize predicted and observed values
%         X_new_norm = (X_new(t_miss(i),v_miss(i)) - Mx(v_miss(i)))./Wx(v_miss(i));
%         X_sm_norm = (Res_old.X_sm(t_miss(i),v_miss(i))- Mx(v_miss(i)))./Wx(v_miss(i));
        X_new_norm = (X_new(t_miss2(i),v_miss2(i)) - Mx(v_miss2(i)))./Wx(v_miss2(i));
        X_sm_norm = (X_old(t_miss2(i),v_miss2(i))- Mx(v_miss2(i)))./Wx(v_miss2(i));
        ...
        size(innov);
    for i=1:size(innov,2)
        forecast(v_miss2(i),1) = Res_old.X_sm(t_miss2(i),v_miss2(i));
        actual(v_miss2(i),1) = Res_new.X_sm(t_miss2(i),v_miss2(i));

    end
        weight=[];t_miss=[];v_miss=[];
    else
%     %----------------------------------------------------------------------
%     %     v_miss=[1:size(X_new,2)]';
%     %     t_miss=t_miss(1)*ones(size(X_new,2),1);
...
% %         X_sm_norm = (Res_old.X_sm(t_miss(i),v_miss(i))- Mx(v_miss(i)))./Wx(v_miss(i));
%         X_new_norm = (X_new(t_miss(i),v_miss(i)) - Mx(v_miss(i)))./Wx(v_miss(i));
%         X_sm_norm = (X_old(t_miss(i),v_miss(i))- Mx(v_miss(i)))./Wx(v_miss(i));
%         
%         % Innovation: Gives [observed] data - [predicted data]
%         innov(i)= X_new_norm - X_sm_norm;         
%     end
...
%         actual(v_miss(i),1) = X_new(t_miss(i),v_miss(i));  
% %         forecast(v_miss(i),1) = Res_old.X_sm(t_miss(i),v_miss(i));
%         forecast(v_miss(i),1) = X_old(t_miss(i),v_miss(i));
%         
%         for j=1:size(v_news,2)
%             singlenews(t_miss(i)-min(t_miss)+1,v_miss(i),j) = temp(1,i,j);
%             weight(v_miss(i),:,j) = gain(:,i,j)/Wx(v_miss(i));
